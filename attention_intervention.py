...
def llama_new_forward(
    ...
    if hasattr(self, "use_cfg"):
        ...
        attn_weights[:, :, -1, text_start_idx:text_end_idx_before_img] = (
                attn_weights[:, :, -1, text_start_idx:text_end_idx_before_img].abs() * self.b + attn_weights[
                                                                                                           :, :, -1,
...
def llama_modify(model, start_layer, end_layer, use_attn, alpha,b, use_cfg,
                 img_start_idx, img_end_idx,text_start_idx,text_end_idx_before_img,text_start_idx_after_img,text_end_idx):

    for i in range(start_layer, end_layer):
        model.model.layers[i].self_attn.use_attn = use_attn
        model.model.layers[i].self_attn.alpha = alpha
        model.model.layers[i].self_attn.b = b
        ...
        model.model.layers[i].self_attn.text_start_idx = text_start_idx
        model.model.layers[i].self_attn.text_end_idx_before_img = text_end_idx_before_img
        model.model.layers[i].self_attn.text_start_idx_after_img = text_start_idx_after_img
        model.model.layers[i].self_attn.text_end_idx = text_end_idx
